...
#painel do usuario.
    ...
    print(Fore.BLUE + f"Olá, {nome}, é um prazer lhe ajudar com a sua conversão de moeda!")

#loop até que a conversao seja bem sucedida.
    while True: 
        moeda_orig = input(Fore.BLUE + "Digite a moeda de origem (Ex: BRL, USD, EUR): ").strip().upper()

#solicita a moeda de destino.
        moeda_dest = input(Fore.BLUE + "Digite a moeda de destino (Ex: BRL, USD, EUR): ").strip().upper()
...
#exibe o resultado da conversao
            print(Fore.BLUE + f"{valor} {moeda_orig} é igual a {result} {moeda_dest}") 

        except ValueError:
#exibe mensagem de erro caso o usuario insira um valor invalido.
            print(Fore.RED + "Erro: Valor inválido. Certifique-se de digitar um número.")  
            continue
...
#pergunta ao usuario se deseja continuar ou sair.
        while True:
            resposta = input(Fore.BLUE + f"Olá, {nome}, deseja fazer uma nova conversão? (S/N): ").strip().upper()

#encerra o programa caso o usuario escolha "N"
            if resposta == "N":
                print(Fore.BLUE + f"Obrigado por usar o conversor, {nome}! Até mais!")
                exit()  
...
#exibe erro caso a entrada não seja válida
                ...
                print(Fore.RED + "Opção inválida! Digite 'S' para continuar ou 'N' para sair.")
